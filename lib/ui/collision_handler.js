...
class CollisionHandler {
  ...
  checkLaserBeamCrabCollisions(beam) {
    ...
    if (crab && beam.overlaps(crab)) {
      if (crab.getY() > 500) {
        game.incrementScore(CRAB_HIT_SCORE_CLOSE);
      } else if (crab.getY() > 400) {
        game.incrementScore(CRAB_HIT_SCORE_MIDDLE);
      } else {
        game.incrementScore(CRAB_HIT_SCORE_FAR);
      }
      ...
      if (game.started) {
        if (crab) {
          if (explosion.overlaps(crab)) {
            game.removeCrab();
            game.incrementScore(CRAB_HIT_SCORE_CLOSE / 2);
          }
        }
        ...
        segments.forEach((segment, idx) => {
          if (explosion.overlaps(segment)) {
            ...
            if (segment instanceof Head) {
              game.incrementScore(HEAD_HIT_SCORE / 2);
            } else {
              game.incrementScore(SEGMENT_HIT_SCORE / 2);
            }
          }
        });
        shrimps.forEach((shrimp, idx) => {
          if (explosion.overlaps(shrimp)) {
            ...
            game.incrementScore(SHRIMP_HIT_SCORE / 2);
          }
        });
      }
    });
