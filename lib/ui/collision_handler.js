...
class CollisionHandler {
    ...
    for (let i = 0; i < sponges.length; i++) {
      if (beam.overlaps(sponges[i])) {
        ...
        if(sponges[i].hits === 0) {
          ...
          this.game.soundHandler.playSeaSpongeDestroy();
          return i;
        } else {
          this.game.soundHandler.playSeaSpongeHit();
          return true;
        }
      }
    }
  ...
  checkLaserBeamSegmentCollisions(beam) {
    ...
    for (let i = 0; i < segments.length; i++) {
      if (beam.overlaps(segments[i])) {
        ...
        game.soundHandler.playSegmentHit();
        return i;
      }
    }
    return false;
  ...
  checkLaserBeamSpiderCollisions(beam) {
    ...
    if (spider && beam.overlaps(spider)) {
      ...
      game.soundHandler.playSpiderHit();
      return true;
    }
    return false;
  }
  ...
  checkLaserBeamShrimpCollisions(beam) {
    ...
    for (let i = 0; i < shrimp.length; i++) {
      if (beam.overlaps(shrimp[i])) {
        ...
        game.soundHandler.playShrimpHit();
        return i;
      }
    }
    return false;
