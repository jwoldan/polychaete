...
export const SPIDER_MIN_VELOCITY = 4;
...
class Spider extends MovingObject {
  constructor(options = {}) {
    ...
    this.maxVelocity = options.maxVelocity || SPIDER_MIN_VELOCITY;
  ...
  updateDecisionPoint() {
    const decisionXDelta = SPIDER_MIN_VELOCITY * getRandomInt(10, 30);
    ...
    if (this.decisionPointY < DIVER_MIN_Y) {
      this.decisionPointY =
        DIVER_MIN_Y + (SPIDER_MIN_VELOCITY * getRandomInt(1, 50));
      if (this.decisionPointY > this.moveBounds.maxY) {
        this.decisionPointY = this.moveBounds.maxY - this.getHeight();
      }
    } else {
      this.decisionPointY =
        DIVER_MIN_Y - (SPIDER_MIN_VELOCITY * getRandomInt(20, 50));
    }
  }
  ...
  updateVelocities() {
    ...
    const velocity = getRandomInt(SPIDER_MIN_VELOCITY, this.maxVelocity);
